...
`parser-exercise.scm` を開いて、`???` の部分を実装してください。
...
1. **エラーメッセージの改善**: 括弧の対応が間違っている場合、より分かりやすいエラーメッセージを表示してください。

2. **ドット記法**: ペア（例: `(a . b)`）を解析できるように拡張してください。

3. **クォート**: `'` 記法（例: `'x`）を `(quote x)` に展開できるように拡張してください。
