...
### このステップで書くもの（ゴール）
- クォート/準クォートを使って、以下の“式生成”ユーティリティを作る:
  - `quote-list`: 与えた3要素から“リスト式”を組み立てる
  - `make-call`: シンボルと引数リストから“関数呼び出し式”を作る
  - `let-binding`: 1変数の`let`式をテンプレートで作る
  - `alist->hash-expr`: 連想リストから`(hash 'k v ...)`を作る
...
(quote-list 'x 'y 'z) ; => '(x y z)
(make-call '+ '(1 2 3)) ; => '(+ 1 2 3)
(let-binding 'x 10 '(* x x)) ; => '(let ((x 10)) (* x x))
(alist->hash-expr '((a 1) (b 2))) ; => '(hash 'a 1 'b 2)
```
